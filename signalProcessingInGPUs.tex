...
CUDA gives each thread launched in a GPU kernel a unique indices called threadIdx and blockIdx.
...
Both threadIdx and blockIdx are three dimensional (i.ei , they both have x, y, and z components).
...
The fastest memory is private local memory in the form of registers, L1 cache and shared memory.
Local memory is fast but only kilobytes are available.
The slowest memory is public memory in the form of L2 cache and global memory.
...
Sections \ref{sec:hardware} and \ref{sec:oqpsk_detector} show the PAQ system has one signal length, $N = \Lpkt = 12672$ samples and two filter lengths $L = L_\text{df} = 21$ and $L = L_\text{eq} = 186$.
...
time-domain convolution using batch processing and shared memory is fastest for $21$ tap filters.
\begin{figure}
...
\begin{figure}
	...
	\caption{Comparison of complex convolution using batch processing on a GPU. The signal length is variable and the filter is fixed at $21$ taps.}
...
\end{table}
\begin{table}
\caption{Convolution using batch processing execution times with for a $12672$ sample signal and $21$ tap filter on a Tesla K40c GPU.}
\begin{center}
\begin{tabular}{lll}
	\toprule
	Algorithm 				& Function or Library		& Execution Time (ms) \\ \midrule
...
The first time-domain convolution produces a composite filter from the convolution of the $186$ sample equalizer filter with the $21$ tap detection filter.
The second time-domain convolution applies the composite $206 = 186 + 21 - $ tap filter to a $12672$ sample signal.
